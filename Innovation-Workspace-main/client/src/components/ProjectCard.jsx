...
export default function ProjectCard({
  ...
  const roleInProject = (() => {
    ...
    const mentorName = String(project.mentorName || project.mentor || project.mentorEmail || "").trim().toLowerCase();
    ...
    if ((currentEmail && currentEmail === mentorEmail) || (currentName && currentName === mentorName)) {
      return "Mentor";
    }
  ...
  return (
    <motion.div whileHover={{ y: -5 }} transition={{ duration: 0.25, delay: index * 0.03 }} className="h-full">
      <Card className="group flex h-full flex-col p-4 transition-all duration-300 hover:shadow-[0_24px_60px_rgba(15,23,42,0.16)]">
        <div className="mb-4 h-1.5 rounded-full bg-[#1e3a8a]" />

        <h3 className="heading-font text-[1.05rem] font-semibold text-slate-900">{project.title || project.name}</h3>

        <div className="mt-2 flex flex-wrap items-center gap-2">
          <StatusBadge status={status} />
          <Badge className="bg-slate-100 text-slate-700">{members.length} Members</Badge>
          <Badge className="bg-slate-900/5 text-slate-700">Your role: {roleInProject}</Badge>
        </div>

        <p className="mt-2 text-sm text-slate-500">{project.department}</p>
        ...
        <div className="mt-auto pt-4">
          <Link
            ...
            className="glass-button-primary inline-flex items-center rounded-full px-4 py-2 text-sm font-semibold text-white transition hover:-translate-y-0.5"
          >
            Enter Workflow -&gt;
          </Link>
        </div>

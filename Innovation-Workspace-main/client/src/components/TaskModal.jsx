...
export default function TaskModal({
  ...
  const normalizedRole = String(currentUserRole || "student").toLowerCase();
  const isMember = isMemberRole(normalizedRole);
  const canApproveDaily = canApproveDailyReports(normalizedRole);
  ...
  const taskStatus = String(task?.status || "todo").toLowerCase();
  const canSubmitTask = isMember && taskStatus === "todo";
  const canReviewDailyTask = taskType === "daily" && canApproveDaily && taskStatus === "submitted";
  const canReviewWeeklyTask = taskType === "weekly" && canApproveWeekly && taskStatus === "submitted";
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div>
                    ...
                    {isEditing ? (
                      ...
                      <p className="glass-surface rounded-2xl px-4 py-3 text-sm text-slate-700">
                        {formatDisplayName(task.assignee, "Unassigned")}
                      </p>
                    )}
                  </div>
                </div>
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="flex items-center justify-between gap-4">
                  <span className="inline-flex items-center gap-1.5 text-slate-500">
                    Approval
                    <InfoTip text="Approval means the submission has been reviewed by the correct approver for that task type." />
                  </span>

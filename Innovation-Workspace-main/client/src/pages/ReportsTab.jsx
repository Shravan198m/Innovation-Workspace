...
function normalizeReportStatus(status) {
  ...
  if (value === "approved" || value === "final_approved" || value === "completed") {
  ...
  if (["submitted", "tl_approved", "rejected"].includes(value)) {
    return value;
  }
...
export default function ReportsTab({ projectId, currentUserRole, currentUserName = "", currentUserEmail = "" }) {
  const normalizedRole = normalizeRole(currentUserRole);
  const isMember = canSubmitReports(normalizedRole) || isMemberRole(normalizedRole);
  const canReviewDaily = canApproveDailyReports(normalizedRole);
  const canReviewWeekly = normalizedRole === "team_lead";
  const canApproveWeekly = canApproveWeeklyReports(normalizedRole);
  ...
  useEffect(() => {
    fetchReports();
    fetchTodoTasks();
  }, [projectId, currentUserName, currentUserEmail]);
  ...
  const fetchTodoTasks = async () => {
    try {
      ...
      const selectableTasks = allTasks.filter((task) => {
        const status = String(task.status || "").toLowerCase();
        const assignee = String(task.assignee || task.assignedTo || "").trim().toLowerCase();
        const matchesMember = !assignee || assignee === String(currentUserName || "").trim().toLowerCase() || assignee === String(currentUserEmail || "").trim().toLowerCase();

        return ["todo", "rejected"].includes(status) && matchesMember;
      });
      setTodoTasks(selectableTasks);
      setFormData((prev) => {
        const matchingTasks = selectableTasks.filter((task) => String(task.taskType || task.type || "DAILY").trim().toUpperCase() === String(prev.type || "DAILY").trim().toUpperCase());

        if (prev.taskId && matchingTasks.some((task) => String(task.id) === String(prev.taskId))) {
          return prev;
        }

        return { ...prev, taskId: matchingTasks[0] ? String(matchingTasks[0].id) : "" };
      });
    } catch {
      setTodoTasks([]);
    }
  ...
  const handleTeamLeadApprove = async (reportId) => {
    if (!canReviewDaily && !canReviewWeekly) {
      setMessage("Only team lead can review reports at this stage.");
      return;
    }

    try {
      await api.patch(`/reports/${reportId}/team-lead-approve`);

      fetchReports();
      fetchTodoTasks();
      setMessage("Report reviewed by team lead.");
    } catch (error) {
      setMessage(getApiErrorMessage(error, "Failed to review report."));
    }
  };

  const handleMentorApprove = async (report) => {
    if (!isMentor) {
      setMessage("Only mentor can approve weekly reports.");
      return;
    }

    if (normalizeReportStatus(report.status) !== "tl_approved" || report.type !== "WEEKLY") {
      setMessage("Mentor can approve only team-lead-approved weekly reports.");
      return;
    }

    try {
      await api.patch(`/reports/${report.id}/mentor-approve`);

      fetchReports();
      fetchTodoTasks();
      setMessage("Weekly report approved by mentor. Waiting for manager final approval.");
    } catch (error) {
      setMessage(getApiErrorMessage(error, "Failed to mentor-approve report."));
    }
  };

  const handleManagerApprove = async (report) => {
    if (!isManager) {
      setMessage("Only manager can do final approval.");
      return;
    }

    if (normalizeReportStatus(report.status) !== "tl_approved" || !report.mentorApprovedAt || report.type !== "WEEKLY") {
      setMessage("Manager can approve only mentor-approved weekly reports.");
      return;
    }

    try {
      await api.patch(`/reports/${report.id}/manager-approve`);
      fetchReports();
      fetchTodoTasks();
      setMessage("Weekly report completed. Final approval marked.");
    } catch (error) {
      setMessage(getApiErrorMessage(error, "Failed to complete report."));
    }
  };

  const handleReject = async (report, mentorComment) => {
    if (!(canReviewDaily || canReviewWeekly || isMentor || canApproveWeekly)) {
      setMessage("You cannot reject reports.");
      return;
    }

    try {
      await api.patch(`/reports/${report.id}/reject`, {
        mentorComment,
      });
      fetchReports();
      fetchTodoTasks();
  ...
  return (
    <div className="space-y-6">
      <Card className="overflow-hidden rounded-[28px] p-0">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">
                Daily reports are reviewed by the team lead. Weekly reports move through team lead, mentor, and manager approval.
              </p>
              <p className="mt-2 text-xs font-medium text-slate-500">
                Members submit reports, team leads review daily work, and weekly work is finalized by mentor plus manager.
              </p>
              <p className="mt-1 text-xs text-slate-500">Each report must be linked to a To Do task.</p>
            </div>

            {canSubmitReports(normalizedRole) && (
              <button
        ...
        <div className="grid gap-3 border-t border-white/35 bg-white/28 px-6 py-5 sm:grid-cols-4 sm:px-8 backdrop-blur-md">
          {[
            ...
            { label: "TL Approved", value: reports.filter((report) => normalizeReportStatus(report.status) === "tl_approved").length },
            { label: "Final Approved", value: reports.filter((report) => normalizeReportStatus(report.status) === "final_approved").length },
          ].map((item) => (
      ...
      {showForm && canSubmitReports(normalizedRole) && (
            ...
            <div className="grid gap-5 lg:grid-cols-[260px_1fr]">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    const matchingTasks = todoTasks.filter((task) => String(task.taskType || task.type || "DAILY").trim().toUpperCase() === String(nextType).trim().toUpperCase());
                    setFormData({
                      ...formData,
                      type: nextType,
                      taskId: matchingTasks[0] ? String(matchingTasks[0].id) : "",
                    });
                  }}
              ...
              <div>
                ...
                <select
                  ...
                  className="glass-input w-full rounded-2xl px-4 py-3 text-sm"
                >
                  {todoTasks.filter((task) => String(task.taskType || task.type || "DAILY").trim().toUpperCase() === String(formData.type || "DAILY").trim().toUpperCase()).length === 0 ? (
                    <option value="">No To Do task available</option>
                  ) : (
                    todoTasks.filter((task) => String(task.taskType || task.type || "DAILY").trim().toUpperCase() === String(formData.type || "DAILY").trim().toUpperCase()).map((task) => (
          ...
          {["ALL", "DAILY", "WEEKLY"].map((type) => (
            <button
              ...
              className={`rounded-full px-4 py-2 text-sm font-medium transition-all ${
                filterType === type
                  ? "bg-[#1e3a8a] text-white shadow-[0_12px_24px_rgba(30,58,138,0.3)]"
          ...
          {filteredReports.map((report) => (
            <Card key={report.id} className="app-elevate rounded-[24px] p-5">
              {(() => {
                ...
                const teamLeadApproved = status !== "submitted" && status !== "rejected";
                const mentorApproved = status === "final_approved" || (status === "tl_approved" && report.type === "WEEKLY" && Boolean(report.mentorApprovedAt));
                const managerApproved = status === "final_approved";

                return (
                  <div className="mb-3 flex flex-wrap items-center gap-2">
                    <span className={`inline-flex items-center rounded-full border px-3 py-1 text-xs font-semibold ${teamLeadApproved ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-slate-200 bg-slate-50 text-slate-600"}`}>
                      Team Lead {teamLeadApproved ? "✓" : "○"}
                    </span>
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-2 text-sm text-slate-500">
                    📅 {new Date(report.createdAt).toLocaleDateString()} • 👤 {report.submittedBy || report.userName}
                  </p>
                </div>

                {canReviewDaily && normalizeReportStatus(report.status) === "submitted" && report.type === "DAILY" && (
                  <div className="flex gap-2">
                    <button
                      onClick={() => handleTeamLeadApprove(report.id)}
                ...
                {canReviewWeekly && normalizeReportStatus(report.status) === "submitted" && report.type === "WEEKLY" && (
                  <div className="flex gap-2">
                    <button
                      onClick={() => handleTeamLeadApprove(report.id)}
                ...
                {isMentor && normalizeReportStatus(report.status) === "tl_approved" && report.type === "WEEKLY" && (
                  <div className="flex gap-2">
                    <button
                      onClick={() => handleMentorApprove(report)}
                      className="inline-flex items-center rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm font-semibold text-emerald-700 transition hover:bg-emerald-100"
                    >
                      Mentor Review
                    </button>
                ...
                {isManager && normalizeReportStatus(report.status) === "tl_approved" && report.type === "WEEKLY" && report.mentorApprovedAt && (
                  <button
                    onClick={() => handleManagerApprove(report)}
                    className="inline-flex items-center rounded-xl border border-cyan-200 bg-cyan-50 px-3 py-2 text-sm font-semibold text-cyan-700 transition hover:bg-cyan-100"
                  >
              ...
              {normalizeReportStatus(report.status) === "tl_approved" && report.type === "WEEKLY" && (
                <div className="mt-4 rounded-2xl border border-cyan-200 bg-cyan-50/80 p-4 text-sm text-cyan-800">
                  {report.mentorApprovedAt ? "Mentor approved. Waiting for manager final approval." : "Team lead approved. Waiting for mentor approval."}
                </div>
              )}

              {normalizeReportStatus(report.status) === "final_approved" && (
      ...
      {rejectingReportId && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-slate-950/35 px-4 backdrop-blur-sm">
          <Card className="w-full max-w-lg rounded-[24px] p-5">
            ...
            <p className="mt-1 text-sm text-slate-500">Enter reason for rejection. This is mandatory and shown to the member.</p>
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  const reportToReject = reports.find((report) => String(report.id) === String(rejectingReportId));
                  await handleReject(reportToReject || { id: rejectingReportId }, rejectReason.trim());
                  setRejectingReportId(null);
                  setRejectReason("");
                }}

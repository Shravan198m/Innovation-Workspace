...
router.post("/:projectId", requireRole("admin"), async (req, res) => {
  ...
  try {
    ...
    if (String(result.rows[0].email || "").trim()) {
        ...
        [
          {
            ...
            link: `${String(process.env.FRONTEND_URL || process.env.CLIENT_URL || "http://localhost:3000").replace(/\/$/, "")}/projects/${projectId}/board`,
          },
        ],
        {
          projectId,
          projectName: projectResult.rows[0].name,
          source: "team-member:create",
        }
      );
    } else {
      console.log("[email] team member added without email; skipping notification:", {
        projectId,
        projectName: projectResult.rows[0].name,
        memberName: result.rows[0].name,
      });
    }
